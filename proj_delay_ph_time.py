...
def delay_time(ph_array):
    delay_time = ph_array["Sleep"] # Устанавливаем время задержки
    global prev_hour, prev_minute, prev_second,prev_pause 
    ...
    if prev_hour > curr_hour: # Если с прошлого измерения закончился день, добавляем к  времени паузы 24 часа т.к ноль находится в прошлом дне 
        pause_time = (curr_hour+24)*3600 + curr_minute*60 + curr_second
    else:
